...
const RegistroFactura = () => {
  ...
  const obtenerDetalleVentas = async () => {
    try {
      const response = await axios.get("http://localhost:5000/detalle-ventas");
  ...
  return (
    <div className="container" style={{ textAlign: "center" }}>
      ...
      <h2>Detalle de venta por producto:</h2>
      <div>
      <div>
        <table
          style={{
        ...
        >
          ...
          <tbody>
            ...
            {detalleVentas.map((producto) => (
              <tr key={producto.PRO_ID}>
                <td>{producto.PRO_NOMBRE}</td>
                <td>{producto.NumeroDeVentas}</td>
                <td>{producto.PrecioTotalVentas}</td>
              </tr>
            ))}
          </tbody>
        </table>
