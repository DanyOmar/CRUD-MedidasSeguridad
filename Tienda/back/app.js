...
app.post("/clientes", async (req, res) => {
  ...
  nuevoCliente.password = hashedPassword;
...
app.get("/detalle-ventas/:inicio/:final", (req,res) => {
  const {inicio,final} = req.params;
  const sql = `
    SELECT
      P.PRO_NOMBRE,
      COUNT(D.DET_ID) AS NumeroDeVentas,
      SUM(D.DET_PRECIOTOTAL) AS PrecioTotal
    FROM
      PRODUCTO P
    INNER JOIN
      DETALLE D ON P.PRO_ID = D.PRO_ID
    GROUP BY
      P.PRO_NOMBRE;
  `;
});

app.get('/detalle-ventas', (req, res) => {
  const query = `
    SELECT
      P.PRO_NOMBRE,
      COUNT(D.DET_ID) AS NumeroDeVentas,
